		            ...
		            <br><br><br>
						...
						<h2><?php echo utf8_encode($datos['titulo_articulo']); ?></h2>
							...
							<?php
								...
								$sqlLikes = "SELECT COUNT(id_likes) FROM tbl_likes WHERE articulo_likes=".$_REQUEST['idB'];
								$artLikes = mysqli_query($con,$sqlLikes);
								$datosLikes = mysqli_fetch_array($artLikes);

								$sqlLinkLikes = "SELECT*FROM tbl_likes WHERE articulo_likes=$_REQUEST[idB] AND usuario_likes=$_SESSION[id]";
								$artLinkLikes = mysqli_query($con,$sqlLinkLikes);
								if($datosLinkLikes = mysqli_num_rows($artLinkLikes) != 0){
									echo "<br/><a href='../conexion/like.proc.php?idB=$_REQUEST[idB]'>No me gusta</a>";
								}else{
									echo "<br/><a href='../conexion/like.proc.php?idB=$_REQUEST[idB]'>Me gusta</a>";
								}
